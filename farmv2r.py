#!/usr/bin/env python3
# farmv2r: v2-флоу (guerrilla OTP + takekey), домен sharklasers, 1 вкладка = 1 хром
...
try:
    ...
    pass
KEYS = os.environ.get("FKEYS", "/tmp/farm/farmv2r_keys.txt")
...
def guerrilla_sess(user):
    s = requests.Session()
    r = s.get("https://api.guerrillamail.com/ajax.php", params={"f": "get_email_address", "lang": "en"}, timeout=15).json()
    sid = r["sid_token"]
    s.get("https://api.guerrillamail.com/ajax.php", params={"f": "set_email_user", "email_user": user, "lang": "en", "sid_token": sid}, timeout=15)
    return s, sid

def maildrop_inbox(user):
    try:
        s, sid = guerrilla_sess(user)
        _gsid[user] = (s, sid)
        r = s.get("https://api.guerrillamail.com/ajax.php", params={"f": "get_email_list", "offset": 0, "sid_token": sid}, timeout=15).json()
        out = []
        for m in r.get("list") or []:
            out.append({"id": m.get("mail_id"), "mailfrom": m.get("mail_from", ""), "subject": m.get("mail_subject", ""), "date": ""})
        return out
    except Exception:
        return []
...
def maildrop_msg(user, mid):
    try:
        s, sid = _gsid.get(user, guerrilla_sess(user))
        r = s.get("https://api.guerrillamail.com/ajax.php", params={"f": "fetch_email", "email_id": mid, "sid_token": sid}, timeout=15).json()
        return {"html": r.get("mail_body", "") or "", "data": ""}
    except Exception:
        return {}
    ...
    while _t.time() - t0 < tmo:
        ...
        for _, m in cands:
            if "crusoe" in (m.get("mailfrom", "") + m.get("subject", "")).lower():
                ...
                if mm and mm.group(1) not in ("999999", "000000", "123456"):
...
async def _flow(wsurl, idx, email, pw):
    async with websockets.connect(wsurl, max_size=20 * 1024 * 1024, open_timeout=15) as w:
        ...
        try:
            ...
            import os as _os1; _ap = _os1.path.join(_os1.path.dirname(_o.environ.get("FKEYS", "/tmp/farm/farmv2r_keys.txt")) or "/tmp/farm", _os1.path.basename(_o.environ.get("FKEYS", "/tmp/farm/farmv2r_keys.txt")).replace("_keys", "_accs"))
            with open(_ap, "a") as f:
                f.write(f"{email}:{pw}\n")
        except Exception:
            pass
...
def worker(idx):
    while st["alive"] < TARGET:
        ...
        em = "l2" + rnd(9) + "@" + os.environ.get("DOMAIN", "sharklasers.com")
...
def telemetry():
    ...
    try:
        ...
        try:
            ap = _o.path.join(_o.path.dirname(_o.environ.get("FKEYS", "/tmp/farm/farmv2r_keys.txt")) or "/tmp/farm", _o.path.basename(_o.environ.get("FKEYS", "/tmp/farm/farmv2r_keys.txt")).replace("_keys", "_accs"))
            with open(ap) as f:
...
if __name__ == "__main__":
    log(f"=== FARMV2R СТАРТ: инстансов {INSTANCES}, цель {TARGET}, домен " + os.environ.get("DOMAIN", "sharklasers.com"))
    save_stats("старт")
